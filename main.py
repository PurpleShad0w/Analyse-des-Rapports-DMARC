...
parser.add_argument("-i", "--initialize", action="store_true", help="create brand new MySQL database")
parser.add_argument("-o", "--overwrite", action="store_true", help="delete previous MySQL database")
parser.add_argument("-s", "--show", action="store_false", help="show the database and Grafana response")
...
def upload_data(data, create_db = False, overwrite = False, show = True):

    # TO FIX
    if overwrite:
        mydb = mysql.connector.connect(host="localhost", user=user, password=pwd, database='DMARC')
        mycursor = mydb.cursor()
        mycursor.execute('DROP TABLE RUA')
        mycursor.execute('DROP TABLE RUF')
        mydb.close()
        create_db = False
...
def update_dashboard(show = True):
...
data = gather_data()
database = upload_data(data, config['initialize'], config['overwrite'], config['show'])
